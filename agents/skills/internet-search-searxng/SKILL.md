...
```
python "%USERPROFILE%/.agents/system/scripts/query_searxng.py "{your search query here}" {number_results}
```
...
```
python "%USERPROFILE%/.agents/system/scripts/query_searxng.py --url "{URL_from_initial_results}"
```
...
```
python "~/.agents/system/scripts/query_searxng.py "{your search query here}" {number_results}
```

Then report back the results. If you need more detailed information from specific sources that were truncated, you can retrieve full content using:

```
python "~/.agents/system/scripts/query_searxng.py --url "{URL_from_initial_results}"
```
...
This skill uses the globally available system scripts that are linked from $HOME/.agents/system/. The scripts are automatically managed by the system setup and are available from any project context.
