...
**Relative path:** `agents/skills/new-assignment/SKILL.md`
**Home directory path:** `~/.agents/skills/new-assignment/SKILL.md`
...
### Path Handling
**Tool Selection for Home Directory Paths**: All file operations targeting `~/.agents/` must use the `execute_command` tool with shell commands, NOT the `list_files`, `read_file`, or `search_files` tools. Those tools only accept workspace-relative paths and cannot expand tilde (`~`) to the home directory.
...
- Windows (PowerShell): `command "$HOME\.config\opencode\path"`
```
- Unix/Linux/macOS: `command ~/.agents/path`
- Windows (PowerShell): `command "$HOME\.config\opencode/path"`
```
...
- Verify the `~/.agents/plans/` and `~/.agents/context/` directories exist using execute_command with the directory existence check pattern from `~/.agents/system/skills/cross-platform-commands.md`.
...
- Check Git branches using execute_command with platform-specific commands:
  ...
  - Windows PowerShell: `git branch --list` (git works in PowerShell) or use the pattern from `~/.agents/system/skills/cross-platform-commands.md`
...
- Read README, AGENTS.md rules, and local dev instructions
...
- For repositories within the workspace: use the search_files tool with:
  ...
  - file_pattern: (optional) filter by file extension
- For repositories outside the workspace: use execute_command with platform-specific search commands (e.g., grep, findstr, Select-String) as defined in ~/.agents/system/skills/cross-platform-commands.md to search those paths
...
- This defines when to research, how to construct safe queries, security constraints, and documentation requirements
 - Invoke the internet-search-searxng skill following its defined workflow at: `~/.agents/skills/internet-search-searxng/SKILL.md`
...
10. Create Implementation Plan
 - Verify the template exists at `~/.agents/skills/new-assignment/references/plan-template.md` using the file existence check pattern from `~/.agents/system/skills/cross-platform-commands.md`.
 - If template is missing, see Error Handling section below.
 - Read the template content from `~/.agents/skills/new-assignment/references/plan-template.md` using the file reading pattern from `~/.agents/system/skills/cross-platform-commands.md`.
 - Copy the plan template, replace placeholders ({ASSIGNMENT_NUMBER}, {Title}, etc.).
 - Save the plan to `~/.agents/plans/{ASSIGNMENT_NUMBER}_{SHORT_NAME}.plan.md` using execute_command with the file writing pattern from the cross-platform file.
...
## References
 - Plan template: ~/.agents/skills/new-assignment/references/plan-template.md
