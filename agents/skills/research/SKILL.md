...
**Skill name:** `research`
**Relative path:** `agents/skills/research/SKILL.md`
**Home directory path:** `~/.agents/skills/research/SKILL.md`
(Expands to absolute path: Unix/Linux/macOS: `/home/user/.agents/...`, Windows: `C:\Users\user\.agents\...`)
...
**IMPORTANT**: Paths like `~/.agents/system/skills/engineering-principles.md` are **executable paths** pointing to where system files are located. These paths must be used in `execute_command` calls to access shared resources. They are absolute paths anchored at the user's home directory.

### File Storage Location
**IMPORTANT**: All files are stored under `~/.agents/`:
- **Context files**: `~/.agents/context/`
- **System files** (shared): `~/.agents/system/`

All references to `~/.agents/` throughout this skill refer to the home directory path appropriate for the operating system.

### Path Handling
**Tool Selection for Home Directory Paths**: All file operations targeting `~/.agents/` must use the `execute_command` tool with shell commands, NOT the `list_files`, `read_file`, or `search_files` tools. Those tools only accept workspace-relative paths and cannot expand`) to the home tilde (`~ directory.
...
**Example Pattern**:
```
- Unix/Linux/macOS: `command ~/.agents/path`
- Windows (PowerShell): `command "$HOME\.agents\path"`
- Windows (PowerShell): `command "$HOME\.agents/path"`
```
...
This skill enforces the engineering principles defined in `~/.agents/system/skills/engineering-principles.md`. The skill's steps and quality checklist ensure each principle is addressed:
...
Reference: `~/.agents/system/skills/engineering-principles.md`
...
- Verify the `~/.agents/context/` directory exists using execute_command with the directory existence check pattern from `~/.agents/system/skills/cross-platform-commands.md`.
...
- Search for existing context files matching the pattern `*{TOPIC_OR_ASSIGNMENT}*.md` using the pattern searching commands. Example:
  - Unix/Linux/macOS: `ls ~/.agents/context/*{SANITIZED_TOPIC}*.md 2>/dev/null`
  - Windows PowerShell: `Get-ChildItem "$HOME\.config\opencode\context" -Filter "*{SANITIZED_TOPIC}*.md" -ErrorAction SilentlyContinue | Select-Object Name`
...
- For each repository involved:
  ...
  - For repositories outside the workspace: use execute_command with platform-specific search commands (e.g., grep, findstr, Select-String) as defined in ~/.agents/system/skills/cross-platform-commands.md to search those absolute/relative paths
- Document file locations, key classes/functions, and search patterns

4. Research External Best Practices (if applicable)
- Applicability: Perform external research when working with frameworks, libraries, or technologies where current best practices matter (see: ~/.agents/system/skills/external-research.md for full criteria)
- Follow the standard pattern defined in: ~/.agents/system/skills/external-research.md
- This defines when to research, how to construct safe queries, security constraints, and documentation requirements
 - The internet-search-searxng skill is located at: `~/.agents/skills/internet-search-searxng/SKILL.md`
...
- Verify the template exists at `~/.agents/skills/research/references/context-template.md` using the file existence check pattern from `~/.agents/system/skills/cross-platform-commands.md`.
- If template is missing, see Error Handling section above.
- Read the template content from `~/.agents/skills/research/references/context-template.md` using the file reading pattern from `~/.agents/system/skills/cross-platform-commands.md`.
- Copy the context template and replace placeholders, fill in ALL sections.
- Save the context file to `~/.agents/context/{TOPIC_OR_ASSIGNMENT}.context.md` using execute_command with the file writing pattern from `~/.agents/system/skills/cross-platform-commands.md`.
...
- Reference: ~/.agents/system/skills/context.md
